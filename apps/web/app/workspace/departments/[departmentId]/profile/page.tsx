...
import { getDepartmentDetailsById } from "../../../../../../hris/app/hris/utils/queries/employee-queries";
...
export default async function WorkspaceDepartmentProfilePage({ params }: Props) {
  ...
  const dept = await getDepartmentDetailsById(departmentId);
  if (!dept) redirect("/workspace/departments");

  const initials = (dept.name?.[0] || "D").toUpperCase();

  return (
    <div className="max-w-4xl mx-auto mt-8">
      <ProfileViewer
        title={dept.name}
        subtitle="Department organization unit"
        initials={initials}
        statusBadge={
        ...
        onBack={async () => {
          "use server";
          redirect("/workspace/departments");
        }}
        sections={[
          {
            ...
            fields: [
              ...
              { label: "Description", value: dept.description || "—" },
              {
                ...
                value: dept.createdAt
                  ? new Date(dept.createdAt).toLocaleDateString("en-PH", { dateStyle: "long" })
                  : "—",
              },
            ],
          },
            ...
            customContent: (
              <div className="space-y-2">
                {dept.employees.length === 0 ? (
                  <span className="text-muted-foreground italic text-xs">No employees assigned to this department</span>
                ) : (
